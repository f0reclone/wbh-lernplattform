...
const form = useForm({
    name: user.name,
    email: user.email,
    university_calendar_url: user.university_calendar_url,
});
</script>
...
<template>
    <section>
        ...
        <form @submit.prevent="form.patch(route('profile.update'))" class="mt-6 space-y-6 ">
            ...
            <div>
                <InputLabel for="university_calendar_url" value="Kalendar URL von deiner Universität (I-Calendar)" />

                <TextInput
                    id="university_calendar_url"
                    type="text"
                    class="mt-1 block w-full"
                    v-model="form.university_calendar_url"
                />

                <InputError class="mt-2" :message="form.errors.university_calendar_url" />
            </div>
            ...
            <div class="flex items-center gap-4">
                ...
                >
                    <p v-if="form.recentlySuccessful" class="text-sm text-gray-600 dark:text-gray-400">Gespeichert.</p>
                </Transition>
            </div>
        </form>
    </section>
